...
export default class Tooltip {
   ...
   * @param {HTMLElement} targetNode - The marker node.
   ...
   */
  moveTooltip = (targetNode, tooltipOffsetX, tooltipOffsetY, markerData) => {
    ...
    const markerClientRect = targetNode.getBoundingClientRect();
    const overlayDiv = select(`#map-tooltip-${markerData.id}`);
  ...
  removeExistingTooltips = () => {
    ...
    selectAll(".map-tooltip").each(function (d, i) {
      _remove(d.id);
    });
  };
   ...
   */
  removeTooltip = (id) => {
    const existing = select(`#map-tooltip-${id}`);
    if (existing.node()) {
      ...
      this.events.dispatch("onTooltipHide", {
        ...
        contentId: `#map-tooltip-${data.id}-content`,
   ...
   * @param {object} markerData - The data associated with the marker.
   */
  drawTooltip = (targetNode, tooltipOffsetX, tooltipOffsetY, markerData) => {
    ...
    const overlayDiv = select("body")
      .append("div")
      .attr("class", "map-tooltip tip-down")
      .attr("id", `map-tooltip-${markerData.id}`);

    const content = overlayDiv
      .append("div")
      .attr("class", "map-tooltip-content")
      .attr("id", `map-tooltip-${markerData.id}-content`)
      .html(markerData.name);

    overlayDiv.append("div").attr("class", "map-tooltip-tip");
    overlayDiv.append("div").attr("class", "map-tooltip-tip-shadow");
    ...
    this.events.dispatch("onTooltipShow", {
      ...
      contentId: `#map-tooltip-${markerData.id}-content`,
      contentClass: ".map-tooltip-content",
    });
